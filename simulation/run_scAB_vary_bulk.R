...
option_list <- list(
  ...
  make_option(c("--results_dir"), type = "character", default = "results", 
              help = ""),
  make_option(c("--results_file_name"), type = "character", default = "scAB_output_tmp.csv", 
              help = ""),
  # make_option(c("--UMAP"), type = "logical", default = TRUE, 
  #             help = "run UMAP"),
  make_option(c("--n_repeat"), type = "numeric", default = 10, 
              help = "The number of repeatition"),
  make_option(c("--target_label"), type = "character", default = "6wNT", 
              help = ""),
...
# results_dir = "results"
# results_file_name = "scAB_output_tmp.csv"
...
library(parallel)
set.seed(2023)
scAB_res_list <- mclapply(1:n_repeat, function(j) {
  print(j)
  ...
  for (i in 1:length(num_per_sample_list)) {
    ...
    sc_dataset_tmp <- findSubset(sc_dataset, scAB_Object = scAB_result, tred = 2)
    
    table(sc_dataset_tmp$scAB_Subset1, sc_dataset_tmp$cell_type)
    table(sc_dataset_tmp$scAB_Subset2, sc_dataset_tmp$cell_type)
    
    scAB_loading_scores <- apply(sc_dataset_tmp@meta.data[, grep("loading", colnames(sc_dataset_tmp@meta.data))], 1, max)
    scAB_res[[i]] <-scAB_loading_scores
    
  }
  return(scAB_res)
}, mc.cores = n_repeat)

scAB_res_list <- lapply(scAB_res_list, function(x) {
  names(x) <- num_per_sample_list
  x
})

scAB_res_list <- lapply(seq_along(num_per_sample_list), function(x) {
  lapply(scAB_res_list, function(l) {
    l[[x]]
  })
})
library(pROC)
auc_res <- lapply(scAB_res_list, function(l) {
...
saveRDS(scAB_res_list, file = file.path(results_dir, paste0(dataset_name, "_scAB_output_vary_nBulk_pure.rds")))
...
saveRDS(scAB_res_list, file = file.path(results_dir, paste0(dataset_name, "_scAB_output_vary_nBulk.rds")))
...
saveRDS(scAB_res_list, file = file.path(results_dir, paste0(dataset_name, "_scAB_output_vary_nBulk_varyCellProp.rds")))
...
saveRDS(scAB_res_list, file = file.path(results_dir, paste0(dataset_name, "_scAB_output_vary_nBulk_varyCellProp_confound.rds")))
