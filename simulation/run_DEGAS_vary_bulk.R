...
option_list <- list(
  ...
  make_option(c("--results_dir"), type = "character", default = "results", 
              help = ""),
  make_option(c("--results_file_name"), type = "character", default = "degas_output_tmp.csv", 
              help = ""),
...
num_per_sample_list = c(10, 20, 50, 100)
...
# results_dir = "results"
# results_file_name = "DEGAS_output_tmp.csv"
...
library(parallel)
set.seed(2023)
DEGAS_res_list <- mclapply(1:n_repeat, function(j) {
  print(j)
...
DEGAS_res_list <- lapply(DEGAS_res_list, function(x) {
  names(x) <- num_per_sample_list
  x
})

DEGAS_res_list <- lapply(seq_along(num_per_sample_list), function(x) {
  lapply(DEGAS_res_list, function(l) {
    l[[x]]
  })
})
library(pROC)
auc_res <- lapply(DEGAS_res_list, function(l) {
...
saveRDS(DEGAS_res_list, file = file.path(results_dir, paste0(dataset_name, "_DEGAS_output_vary_nBulk.rds")))
